...
class CQEditBase(   ):
    ...
    def get_data_for_record( self, record, record_state ):
        ...
        if not self.is_changed:
            ...
            print( "get_data_for_record not changed so nothing ?? a mistake ERROR !! " )
            return
    ...
    def ct_prior( self   ):
        ...
        """
        msg         = f"ct_prior not implemented yet. \n {str( self ) = }"
        logging.error( msg )
        raise NotImplementedError( msg )
...
class CQTextEdit(QTextEdit, CQEditBase):
    ...
    def set_data_from_record( self, record ):
        """a debug trick to try other than that consider an if  """
        print( "set_data_from_record this for debug only ")
        CQEditBase.set_data_from_record( self, record )
...
# class CQDateEdit( CQEditBase, QDateEdit,   ):  # reverse does not help
    ...
    def __init__(self,
        ...
        raw_data      = self.get_raw_data()
        print( f"raw_data = {raw_data}")    # PyQt5.QtCore.QDate(2000, 1, 1)
        try:
            print( "start try")
    ...
    def setDateHide(self, qdate):
        """
        will this fix incorrect call?  --- no
        never gets callde
        """
        print( "use substitute setDate ")

        # Explicitly call QDateEdit's setDate
