...
# ---- tof
"""
this is just in a very draft state, copied from
structured_text for experiments
...
class FileWriterxxx(  ):
    ...
    def __str__( self, ):
        ...
        string_utils.to_columns( a_str, [ "file_name_to_write",
                                        f"{self.file_name_to_write}"  ] )
        string_utils.to_columns( a_str, [ "file_data",
                                        f"{self.file_data}"  ] )
...
#====================================
class SelectCSVWriterxxx( object ):
    """
...
#====================================
class SelectGUIWriterxxx( object ):
    """
...
#====================================
class SelectTXTWriterxxx( object ):
    """
    from easy_db.py file_writes, in process
    HTML.py - a Python module to easily generate HTML tables and lists | Decalage
        *>url  https://www.decalage.info/python/html
...
class TxtWriter( object ):
    """
    this is first experiment with stuffdb
    """
    #----------- init -----------
    def __init__(self, fileout_name ):
        """
    ...
    #----------- init -----------
    def write_header( self, field_list ):
        """
        Purpose: see title
        Args: Return: state change, output
        Raises: none planned
        """
        self.fileout                = open( self.file_name, "w", encoding = "utf8", errors = 'replace' )
        self.fileout.write( "write_header\n" )


    #----------- init -----------
    def write_item_header( self, field_list ):
        """
        Purpose: see title
        Args: Return: state change, output
        Raises: none planned
        """
        self.fileout.write( "write_item_header\n" )

        for i_field in field_list:
            self.fileout.write( f"{i_field.field_name} >> {i_field.get_raw_data()}\n"  )
    ...
    #----------- init -----------
    def write_item_text( self, text_field ):
        """
        Purpose: see title
        Args: Return: state change, output
        Raises: none planned
        """
        self.fileout.write( "write_item_text\n" )
        self.fileout.write( f"{text_field}\n" )
        text       = text_field.get_raw_data()
        text_lines = string_utils.clean_string_to_list( text )
        for i_line in text_lines:
            self.fileout.write( f"{i_line}\n"  )
