...
KEY_WORDS:      some stuff rsh custom  edit with sql model for forms and fields record stagetwo
CLASS_NAME:     CustomEditWidgetTab
WIDGETS:        CQLineEdit CQDateEdit QSqlTableModel QRecord
...
class CustomEditWidgetTab(  tab_base.TabBase  ) :
    ...
    def _build_gui_form(self, layout  ):
        ...
        edit_field.ct_default      = partial( edit_field.do_ct_value, "defalt" )
    ...
    def check_field_exist( self, record, field_name):
        """
        ...
        if record.indexOf(field_name) != -1:
            ...
            self.append_msg( f"Value for '{field_name}': {value}")
        else:
            self.append_msg( f"Field '{field_name}' does not exist in the record.")
    ...
    def record_to_field(self, record ):
        ...
        """
        self.append_function_msg( f"record_to_field {record = }" )

        for i_field_name in ["joe"]:
            self.check_field_exist( record, i_field_name )

        for i_field_name in self.field_list:
            self.check_field_exist( record, i_field_name )
    ...
    def new_record( self, next_key, a_type ):
        ...
        promoted  --  ok ??
        """
        self.append_function_msg( f"new_record {next_key = } {a_type = }" )
    ...
    def select_by_id( self, a_id   ):
        ...
        taken from stuff and simplifed
        """
        self.append_function_msg( f"select_by_id {a_id = }" )
        ...
        self.append_msg( f"{INDENT}select_by_id{model.rowCount() = }  ")

        self.append_msg(  FIF( model, msg = "select_by_id post filter and select " ) )
    ...
    def update_db( self, ):
        ...
        self.append_function_msg( f"update_db {self.current_record_type = }  " )


        record_state   = self.current_record_type
        if   record_state   == RECORD_NULL:
            self.append_msg( "update_db record null no action, return ")
            return
            ...
            self.append_msg( f"update_db wtf  {self.record_state = } ")
        # if self.key_word_table_name:
    ...
    def update_new_record( self ):
        ...
        """
        self.append_msg( f"DetailTabBase update_new_record  {self.record_state  = }")
    ...
    def inspect(self):
        ...
        the usual
        """
        self.append_function_msg( "inspect tbd" )
