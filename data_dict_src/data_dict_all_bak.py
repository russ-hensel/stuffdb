...
def get_column_value( column, attribute ):
    """
    replace get_value
    """
...
def rpt_sub_tab_columns_order( table_name, verbose = False ):
    ...
    for ix, i_column in enumerate( columns ):
        ...
        col_head_width     = i_column.col_head_width  # get_value( )
    ...
    def to_build_form( self,    ):
        ...
        for ix_column, i_column in enumerate( column_list ):
            ...
            if  True:
                ...
                #                         attribute       = "form_edit",
                #                         current_value   = i_column.form_edit  # .i_column.{field_name}
                #                         )

                # can do with get_attr

                value     = get_column_value( i_column, "form_edit" )

                if value != old_value:
                    print( f"value mismatch { value = }")
                ...
                if form_make_ref:
                    line_list.append( f'{indent_1}self.{i_column.field_name}_field     = edit_field' )
